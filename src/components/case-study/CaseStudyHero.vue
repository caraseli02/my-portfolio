    ...
    <div class="relative max-w-6xl mx-auto px-6 lg:px-8 py-24 w-full">
      <div class="max-w-3xl">
        ...
        <div class="flex items-center gap-2 mb-4">
          ...
          <span class="text-cream-300/60 font-mono text-xs"
            >cat {{ category.toLowerCase().replace(/\s+/g, "-") }}.md</span
          >
        </div>

        <!-- Title -->
        <h1
          class="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white mb-4 leading-tight tracking-tight"
        >
          {{ title }}
        </h1>
        ...
        <div
          class="flex flex-wrap gap-x-8 gap-y-2 text-sm text-cream-300/60 font-mono border-t border-white/10 pt-4"
        >
          <span>{{ duration }}</span>
          <span>{{ role }}</span>
          <span>{{ year }}</span>
